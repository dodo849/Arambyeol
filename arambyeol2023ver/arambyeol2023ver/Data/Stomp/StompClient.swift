...
public final class StompClient: NSObject, URLSessionDelegate {
    ...
    private var receiveCompletions: [String: (Result<StompReceiveMessage, Error>) -> Void] = [:]
        ...
        if let topic = topic {
            ...
            executeReceiveCompletions.forEach { _, receiveCompletion in
                ...
                case .success(let responseMessage):
                    receiveCompletion(.success(responseMessage))
                }
            }
        }
    }
    ...
    private func toResponseMessage(
        ...
        guard let commandString = lines.first,
              let command = StompCommand(rawValue: String(commandString)) else {
